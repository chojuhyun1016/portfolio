...
/**
 * AES-128 CBC 암호화/복호화 엔진
 */
public class Aes128Engine {

    private static final String ALGORITHM = "AES/CBC/PKCS5Padding";
    private static final String KEY_ALGORITHM = "AES";
    private static final int REQUIRED_KEY_LENGTH = 16; // 128-bit
    ...
    private static void validate(byte[] key, byte[] iv) {
        Objects.requireNonNull(key, "AES-128 key must not be null");
        Objects.requireNonNull(iv, "AES-128 IV must not be null");
        ...
        if (iv.length != REQUIRED_IV_LENGTH) {
            throw new IllegalArgumentException("AES-128 IV must be 16 bytes");
        }
    }
}
