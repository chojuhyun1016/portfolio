...
*             비정상일 경우 CD_SOCKET_STAT_ERROR를 반환한다.                   *
*******************************************************************************/
int CDSocketGetStat( int _iSocket )
{
    int     iResult;
    ...
    if( _iSocket < 0 )
        return  CD_SOCKET_STAT_ERROR;

    iResult = CDSocketPoll( &stPoll, 1, NULL );
...
*             여부를 반환한다. 이상이 있을경우 CD_SOCKET_STAT_ERROR(-1)을      *
*             반환하고 소켓이 블럭 되어있거나 데이터가 없을 경우               *
*             CD_SOCKET_STAT_NORMAL(0)을                                       *
*             반환한다.                                                        *
*******************************************************************************/
int CDSocketGetReadable( int _iSocket )
{
    ...
    if( stPoll.revents & ( POLLIN | POLLRDNORM ) )
        return  CD_SOCKET_STAT_READABLE;
...
*             여부를 반환한다. 이상이 있을경우 CD_SOCKET_STAT_ERROR(-1)을      *
*             반환하고 소켓이 블럭 되어있거나 데이터가 없을 경우               *
*             CD_SOCKET_STAT_NORMAL(0)을 반환한다.                             *
*             일정시간(_iTimeout) 만큼 기다려도 읽기 가능상태가 아닐경우       *
*             읽을 데이터가 없음(CD_SOCKET_STAT_NORMAL)을 반환한다.            *
*******************************************************************************/
int CDSocketGetTimedReadable( int _iSocket, int _iTimeout )
{
    ...
    iResult = CDSocketPoll( &stPoll, 1, _iTimeout );
...
int CDSocketGetSendable( int _iSocket )
{
    ...
    if( stPoll.revents & ( POLLOUT ) )
        return  CD_SOCKET_STAT_SENDABLE;

    return  CD_SOCKET_STAT_BLOCK;
}
...
int CDSocketRead( int _iSocket, char* _cpReadBuff, int _iLength, int _iMiliTime, stCDSocketUtilErr* _stpUtilErr )
{
    ...
    while( 1 )
    {
        ...
        if( iRTimer > CDSocketGetElapseMiliTime( &tsStartTime, &tsCurrentTime ) )
            continue;

        break;
    }

    _stpUtilErr->iErrno = errno;

    if( iNewReadBytes >= 0 )
        _stpUtilErr->iReturn = CD_SOCKET_TIME_OUT;
    else
        _stpUtilErr->iReturn = CD_SOCKET_RECV_ERROR;
...
int CDSocketWrite( int _iSocket, char* _cpWriteData, int _iLength, int _iMiliTime, stCDSocketUtilErr* _stpUtilErr )
{
    ...
    if( iNewSendBytes >= 0 )
        ...
        _stpUtilErr->iReturn = CD_SOCKET_SEND_ERROR;
...
*             CPU 를 잡아먹지 않는다.                                          *
*******************************************************************************/
int CDSocketRecv( int _iSocket, char* _cpRecvBuff, int _iLength, int _iMiliTime, stCDSocketUtilErr* _stpUtilErr )
{
    int     iReadBytes = 0;
    int     iNewReadBytes = 0;
    int     iRTimer = (int)_iMiliTime;
    ...
    while( 1 )
    {
        ...
        if( ( iRTimer -= CDSocketGetElapseMiliTime( &tsStartTime, &tsCurrentTime ) ) > 0 )
            continue;
...
*             실패 할 경우 CD_SOCKET_ERROR(-1)을 반환한다.                     *
*******************************************************************************/
int CDSocketGetAddrByHost( char* _cpName, char* _cpBuf, int _iBufLen )
{
    ...
    #ifdef  _SOLARIS_
        stpHost = gethostbyname_r( _cpName, &stHost, caBuf, sizeof( caBuf ), &iError );
    #elif _CENT_OS_
        iResult = gethostbyname_r( _cpName, &stHost, caBuf, sizeof( caBuf ), &stpHost, &iError );
    #else
        stpHost = gethostbyname_r( _cpName, &stHost, caBuf, sizeof( caBuf ), &iError );
    #endif
...
*             실패하거나 에러일 경우 CD_SOCKET_ERROR(-1)를 반환한다.           *
*******************************************************************************/
int CDSocketGetAddrBySock( int _iFd, char* _cpBuf, int _iBufLen )
{
    ...
    if( getpeername( _iFd, (struct sockaddr*)&stAddr, (socklen_t*)&iLen ) < 0 )
        return  CD_SOCKET_ERROR;

    if( inet_ntop( AF_INET, (struct in_addr*)&stAddr.sin_addr, _cpBuf, _iBufLen ) == NULL )
        return  CD_SOCKET_ERROR;
...
*******************************************************************************/
static int CDSocketGetElapseMiliTime( struct timespec* _tvStrTime, struct timespec* _tvCurTime )
{
    unsigned long ulSec;
