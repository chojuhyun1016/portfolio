package org.example.order.cache.core.redis.impl;

import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.example.order.cache.core.redis.RedisListPopOptions;
 ...
 * 등록은 설정(오토컨피그)에서 @Bean 으로만 수행 (컴포넌트 스캔 미사용)
 */
...
public class RedisRepositoryImpl implements RedisRepository {
    ...
    @Override
    public void transactionAddSet(String key, java.util.Collection<Object> members) {
        redisTemplate.execute(new SessionCallback<>() {
            @Override
            public Object execute(RedisOperations operations) throws DataAccessException {
                operations.multi();
